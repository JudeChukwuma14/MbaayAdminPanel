            ...
            ? {
                ...
                messages: chat.messages.map((msg) =>
                  msg.id === deleteDialog.messageId
                    ? { ...msg, deletedFor: deleteType }
                    : msg
                ),
                pinnedMessageId:
                  chat.pinnedMessageId === deleteDialog.messageId
                    ? undefined
      ...
      prevChats.map((chat) =>
        chat.id === activeChat
          ? {
              ...chat,
              pinnedMessageId:
                chat.pinnedMessageId === messageId ? undefined : messageId,
            }
          : chat
      )
    );
    showFeedback(
      activeChatDetails?.pinnedMessageId === messageId
        ? "Message unpinned"
        : "Message pinned"
    );
  };
  ...
  const handleForwardMessage = (targetChatId: string) => {
    if (forwardDialog.messageId) {
      const messageToForward = activeMessages.find(
        (m) => m.id === forwardDialog.messageId
      );
      if (messageToForward) {
